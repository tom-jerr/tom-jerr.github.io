...
- SP: seq len 切分
- TP: hidden size 切分
  $$ activation\space size:[batch\_size, seq\_len, hidden\_size]$$
...
**Example**
**megatron-v3 中使用 SP+TP 来减小 activation 的大小**[^megatronv3]
...
⚠️ 虽然计算上可行，但每个 GPU 都需要一份完整的 K 和 V，**没有解决长序列带来的巨大 KV Cache 内存开销**，这违背了 SP 的初衷。
...
⚠️ **softmax 计算需要完整的 score 矩阵，内存瓶颈没有解决**
