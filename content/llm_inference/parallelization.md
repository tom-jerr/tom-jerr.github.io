...
😄在分布式训练中，GPU 之间的通信必不可少，所以我们需要知道 NCCL 的基本通信原语[^NCCL]
...
- **ZB-H2**：device 1 的峰值增加到 2 \* 1F1B，bubbles 接近为 0
  ...
  - ⚠️ 这里实际上有一个问题，optimizer 一般需要全局同步，但是 ZB-H2 里每个 device 都有自己的 optimizer state，需要额外的机制来实现
    ![](img/ZB-H2.png)
...
- ⚠️ 下面的图里，有两个 X，代表 bubble，即，**device 2 需要等待 device 3 传递过来 14+0 的执行结果之后，才可以自己执行。**
...
- Pros: **No pipeline bubbles**
- Cons:
  - 😭**只在本地更新权重，没有全局同步**。小数据集上可以很好的泛化，但是大数据集上很难
...
- 💀 **No Memory saving compared to single device case, which is opposite to the purpose of model parrallelism**
...
1. Overlap Computation with Communication: 使用桶划分，DDP 只需要等待同一桶中的所有内容都准备好后才开始通信。**DDP 为每个梯度累加器注册了一个自动微分钩子。钩子在相应的累加器更新梯度后触发**，并将检查它所属的桶。如果同一桶中所有梯度的钩子都已触发，最后一个钩子将触发该桶的异步 AllReduce。
