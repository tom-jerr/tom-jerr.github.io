...
1. **向量化内存访问 (Vectorized Load/Store)：**
...
1. **FlashAttention 的核心思想 (Tiling + Recomputation)：**
...
1. **Mempool (内存池):** 为什么 Graph Capture 要求显存地址是固定的？（因为 Graph 记录了指针地址，如果下一次运行地址变了，Graph 就失效了）。
...
1. **Shared Memory** (作为显式缓存)

1. **Vectorized Access** (`float4` 提升吞吐)

1. **Warp Primitive** (寄存器级通信)
