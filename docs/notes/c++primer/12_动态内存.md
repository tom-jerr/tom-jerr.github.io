...
- 重载了\*和->运算符，由shared_ptr继承使用，使得智能指针最终能拥有和普通指针一样行为，尽管智能指针本质上是一个对象
  从析构函数来看，里面啥也没做，说明接管的普通指针也不是在这里释放的，所以有可能是由_M_refcount来完成释放内存这个工作，下面分析\_\_shared_count的实现
- 将weak_ptr作为友元
...
#### \_\_Sp_counted_base&&\_\_Sp_counted_ptr
