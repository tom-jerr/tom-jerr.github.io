...
- **极致的效率** 🚀：LLM 参数量极大，RMSNorm 去掉一个可训练参数对 FLOPs 减少效果非常明显。可以实现更快的训练速度和更低的推理延迟。
- **性能相当**：这种简化并没有带来明显的性能损失。在 Transformer 这种结构中，去掉均值中心化这一步，模型的性能依然非常稳定和强大。
- **训练稳定性**：RMSNorm 保留了 LayerNorm 最重要的特性——**重新缩放的不变性**。这意味着它依然能够有效地控制层与层之间激活值的尺度（scale），防止梯度爆炸或消失，保证了深度网络训练的稳定性。
...
| ------------------------------------ | ---------- | ----------------- |
| **T5**                               | GeGLU      | 来自[^swiglu]     |
| **PaLM / Qwen2 / LLaMA2-3 / Falcon** | SwiGLU     | 来自[^swiglu]结论 |
| **GPT-NeoX / MPT / Baichuan**        | SwiGLU     | 同源思想          |
...
- A gate linear projection ($W\_{gate}$​).
- An up linear projection ($W\_{up}$​).
- A SiLU activation function applied to the output of $W\_{gate}$
- An element-wise multiplication of the **SiLU-activated $W\_{gate}$ output and the$W\_{up}$ output. This forms the "gated" part**.
- A final down linear projection ($W\_{down}$​).
  $$
  MLP(x) = (SiLu(W\_{gate}(x))\\odot W\_{up}(x))W\_{down}
  $$
