...
- 量化公式（Affine Quantization）：
    ...
    $$Q(x) = \text{clamp}(\text{round}(\frac{x}{S} + Z), Q_{min}, Q_{max})$$
...
- **实现步骤：**
    ...
    2. 读取 KV Cache 进 Attention 计算时，实时转回 FP16。
...
1. **基础：** 熟练写出 $X_{real} = S * X_{int}$，知道 Per-channel 和 Per-token 的区别。
    
2. **算法（懂原理即可）：** 知道 **AWQ** 保护关键权重，**SmoothQuant** 平滑激活值离群点。
