...
1. 并行搜索：类似 iQAN 的思想，使用多入口点进行并行搜索。
...
1. 邻居选择：通过新的方式，比如角度距离等新发表的文章来选择邻居，而不是简单的欧氏距离。
1. 使用量化：1-bit 量化
1. 减少计算量：比如通过概率路由或其他不等式剪枝方式来实现
