...
- 较高值的​​ token 开始的分支更有可能被接受，每一轮只保留最有希望的 k 个 frontier nodes 来扩展树结构。
  - 下一层将这 $K^2$ 个累积得分展平，并在其中挑出得分最高的 $K$ 个
...
- 如果你在全局挑选出累积概率最高的 $N$ 个节点，只要一个子节点被选中了，它的父节点必然因为概率更大而已经被选中了，所以不会存在丢失父亲节点的问题。
...
- EAGLE-2 扩展阶段的目的是加深草图树，但所选择的 token 可能在全局上不如未选择的浅层节点最优。因此，在 reranking 阶段，我们对所有 draft token 进行 reranking。
...
- 问题变成了如何 expand 树，EAGLE-2 通过选择全局接受概率最高的 top-k token 进行扩展来解决这个问题。
...
- **Expand 的目标是将 draft token tree 变深**：只要有一条长路径全被 Target Model 命中，我们就能一次性解码出好几个 Token，从而极大地提升系统吞吐量。在这个阶段，算法允许局部探索那些置信度不算特别高的深层节点。
- **Reranking 的目标是优化最终选择的 draft token**，确保选出的 token 在全局上具有最高的接受概率。
  - SGLang 中实现的时候只是选择了 expand 阶段的 top-k token，kernel 中处理了没有父节点但是有子节点这种情况
> [!NOTE]
>但在 PyTorch 中，默认情况下 torch.topk 遇到完全相同的值（Tie）时，会优先选择**在原张量中位置靠前（索引更小）** 的元素。这保证了操作的稳定性和确定性。**使用 torch.topk 来选择具有相同 Vi 的 token 是正确的**，因为它会自动优先选择较浅的节点（索引更小的节点）。这确保了我们在 expand 阶段选择的 token 满足优先选择较浅节点的要求，无需额外的处理来实现这一点。

> [!WARNING]
> 如果使用高效 TOPK 算子可能需要考虑是否会造成父节点缺失而直接选了子节点的情况
