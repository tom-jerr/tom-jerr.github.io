...
tags:
  - CUDA
created: 2025-11-3
description: 本文探讨了 GPU 内存系统的演进路径，重点介绍了如何通过提升带宽利用率和隐藏延迟来优化 GPU 性能。涵盖了 Little's Law、ILP/DLP 并行扩展、异步内存加载以及启动延迟优化等关键技术。
---
...
- A100: 需要约 1.2 MB 的全局数据在途（In-flight）才能填满带宽。
- H100: 需要约 2 MB - 3 MB 的数据在途。
> 为什么简单 Kernel 填不满？

如果一个线程只负责一个 float（4 字节），一个 SM 满载 2048 个线程只能提供 $2048 \times 4 = 8 \text{ KB}$ 的 In-flight 数据。即使全显卡 100 多个 SM 全部满载，总 In-flight 数据量也只有 1 MB 左右，远远喂不饱 H100 的 3 TB/s 带宽。
...
$$
	\text{bytes-in-flight / SM} = \text{loads/thread} \times \text{bytes/load} \times \text{threads/block} \times \text{blocks/SM}
$$
...
- 提升 ILP：每线程处理多元素、循环展开、增加并行加载（单线程更多独立内存操作）。
  - 通常展开 2 或 4 次。观察 Nsight Compute 中的 Warp State Statistics，如果 Long Scoreboard（等待内存返回）导致的 Stall 很高，说明需要增加 Unroll。
